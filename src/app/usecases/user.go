...
func NewUser(repo IUserRepo, m app.Mailler) *User {
...
func (u *User) SendPasswordResetLink(email, resetLink string) error {
	...
	subs[":resetURL"] = rurl.String()
	u.m.SetTemplate("50c9d176-80a2-424a-9b15-b6df0495acbf", subs)

	if err := u.m.Send(email, "şifremi unuttum", "dummy content"); err != nil {
		return err
	}
